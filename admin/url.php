...
$page=$_GET["page"];
if ($page){
echo msglayerurl("删除成功",8,"url.php?page=$page");
}else{
echo msglayerurl("删除成功",8,"url.php");
}
}
...
$del_sun=count($array); 
for($i=0;$i<$del_sun;$i++){
$sql='select * from  url  where id='.$array[$i];
$rs=mysql_query($sql);
...
mysql_query('Delete from url where id='.$array[$i]); 
echo msglayerurl("删除成功",8,"url.php?$tzurl");
}
}
}else{ 
echo msglayer("请选择你要删除的内容",8);
...
<div class="content">
<div id="divMain">
<div class="divHeader">随机域名列表</div><div class="SubMenu"></div><div id="divMain2">
...
$Page_size=18; 
$sql = "WHERE 1=1";
$result = mysql_query("select id from url ".$sql."  ");
...
$query = mysql_query("select * from  url   ".$sql." order by id desc   limit $offset, $Page_size");
